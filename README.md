                    ...
                    - - **Loss**: Categorical Crossentropy
                      - - **Epochs**: 50
                        - - **Input Size**: 200×200×3
                          ...
                          >
                          > 2. 2. **Transfer Learning** — Pre-trained ImageNet weights are used as the backbone for each CNN architecture.
                          >   
                          >    3. 3. **Fine-tuning** — Models are trained with custom classification heads and optimized using the Adam optimizer with learning rate scheduling.
                          >      
                          >       4. 4. **Explainability Analysis** — The **Average Drop** metric is computed for each model to quantify interpretability. Lower values indicate the model is using more diagnostically relevant features.
                          >         
                          >          5. ### Average Drop Formula
                          >         
                          >          6. $$\text{Average Drop} = \frac{1}{N} \sum_{i=1}^{N} \frac{\max(0, Y_i^c - O_i^c)}{Y_i^c} \times 100\%$$
                          >         
                          >          7. Where:
